...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-gray-50 rounded-lg p-4 border-l-4 border-gray-400">
        ...
        <p class="text-lg font-bold text-gray-900 mt-1">{{ \App\Helpers\FormatHelper::formatIDR($summary['total']) }}</p>
    </div>
...
<div class="bg-white rounded-lg shadow p-6">
    <div class="flex flex-wrap gap-2 justify-between items-center mb-6">
        <h2 class="text-xl font-semibold text-gray-900">
            Invoices
            @if(request('status'))
                <span class="text-sm font-normal text-gray-500">— filtered by: <strong>{{ request('status') }}</strong></span>
            @endif
            @if(request('filter'))
                <span class="text-sm font-normal text-gray-500">— period: <strong>{{ request('filter') }}</strong></span>
            @endif
        </h2>
        {{-- Status filters --}}
        <div class="flex gap-2 text-sm flex-wrap">
            <a href="{{ route('finance.index', ['filter' => request('filter')]) }}"
               class="{{ !request('status') ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }} px-3 py-1 rounded">All</a>
            <a href="{{ route('finance.index', ['filter' => request('filter'), 'status' => 'paid']) }}"
               class="{{ request('status') === 'paid' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700' }} px-3 py-1 rounded">Paid</a>
            <a href="{{ route('finance.index', ['filter' => request('filter'), 'status' => 'sent']) }}"
               class="{{ request('status') === 'sent' ? 'bg-yellow-500 text-white' : 'bg-gray-200 text-gray-700' }} px-3 py-1 rounded">Pending</a>
            <a href="{{ route('finance.index', ['filter' => request('filter'), 'status' => 'overdue']) }}"
               class="{{ request('status') === 'overdue' ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-700' }} px-3 py-1 rounded">Overdue</a>
        </div>
    </div>
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50 transition-colors">
                    ...
                    <td class="py-3 px-4">
                        ...
                        @else
                            <span class="text-gray-500 text-xs">{{ $invoice->booking?->sales?->name ?? '—' }}</span>
                        @endif
                    </td>
                ...
                @empty
                <tr><td colspan="6" class="py-8 text-center text-gray-500">No invoices found</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
